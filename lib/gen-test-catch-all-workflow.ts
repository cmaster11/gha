...
import {
  actionsRemapping,
  ciTestCatchAllWorkflowName
} from './ci/ci-shared.js';
import traverse from 'traverse';

export async function genTestCatchAllWorkflow(
  remapped = false
): Promise<boolean> {
  const outFilePath = path.join(workflowsDir, ciTestCatchAllWorkflowName);
  const currentContent = (await fs.exists(outFilePath))
  ...
  const workflow = {
    ...
    'run-name':
      "${{ inputs.workflow-name != '' && format('Run test workflow: {0}', inputs.workflow-name) || '' }}",
    on: {
      workflow_dispatch: {
        inputs: {
          'workflow-name': {
